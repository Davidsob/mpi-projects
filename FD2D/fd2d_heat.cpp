...
void setBC(vector<double> &u0, LocalGrid &grid, MPI_Comm comm);
...
int main(int argc, char ** argv)
{
    ...
    // get source
    ConstantSource * H = new ConstantSource(T_cold);
    ...
    // init BCS
    dirchletBoundaryCondition * bcW = new dirchletBoundaryCondition(T_cold);
    convectiveCooling * cool_x = new convectiveCooling(h,T_inf,hx);
    convectiveCooling * cool_y = new convectiveCooling(h,T_inf,hy);
    neumannBoundaryCondition * insultation = new neumannBoundaryCondition;
    
    vector<boundaryCondition * > bcs = {
        dynamic_cast<boundaryCondition *>(bcW),
        dynamic_cast<boundaryCondition *>(cool_y),
        dynamic_cast<boundaryCondition *>(cool_x),
        dynamic_cast<boundaryCondition *>(cool_y)
    };
    my_grid.setBoundaryCondtions(bcs);

    // set IC
    setIC(u,hx,hy,C,dynamic_cast<PhysicalSource *>(H), my_grid, MPI_COMM_WORLD);
    
    // set BC
    setBC(u,my_grid,MPI_COMM_WORLD);
...
void setBC(vector<double> &u0,LocalGrid &grid, MPI_Comm comm)
{
    ...
    vector<boundaryCondition * > bcs = grid.getBoundaryConditions();
    
    // set dirchlet
    if(nbr[0] == -1)
        for(i = 0; i <= local_rows; i++) bcs[0]->setBC(u0[grid.idxFromCoord(i, 0)]);
    if(nbr[1] == -1)
        for(i = 0; i <= local_cols; i++) bcs[1]->setBC(u0[grid.idxFromCoord(0, i)]);
    if(nbr[2] == -1)
        for(i = 0; i <= local_rows; i++) bcs[2]->setBC(u0[grid.idxFromCoord(i, local_cols)]);
    if(nbr[3] == -1)
        for(i = 0; i <= local_cols; i++) bcs[3]->setBC(u0[grid.idxFromCoord(local_rows, i)]);
              ...
              LocalGrid &grid,MPI_Comm comm)
{
    ...
    while (t <= t_end) {
        ...
        grid.updateBoundaryConditions(u, neighbors_data,comm);
