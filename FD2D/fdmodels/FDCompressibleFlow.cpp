...
void FDCompressibleFlow::solve(MPI_Comm comm)
{
    ...
    double dt = 0;
    while (this->t <= this->t_end && step_no < 1000) {
...
//        printf("\ndt = %1.4g[s]\n", dt);
        ...
        //        if(rank == 0) printf("...advance mass transport\n");
        mass->advanceSolution(dt, comm);
        
        this->updateData();
        // 2. update velocity field
        //        if(rank == 0) printf("...advance momentum transport\n");
        this->advanceSolution(dt,comm);
        // 3. update temperature
        //         if(rank == 0) printf("...advance heat transport\n");
        energy->advanceSolution(dt, comm);
        
        // 4. set solutions
        //         if(rank == 0) printf("...set mass transport solution\n");
        this->data_manager->getData("density") = this->data_manager->getData("densityp");
        
        //         if(rank == 0) printf("...set momentum solution\n");
        for(auto var : primary_variables)
            this->data_manager->getData(var) = this->data_manager->getData(var + "p");
        
        //         if(rank == 0) printf("...set heat solution\n");
        this->data_manager->getData("temperature") = this->data_manager->getData("temperaturep");
        
//        printf("u:[");
//        for(size_t idx = 0; idx < this->grid->getNumberOfGridPoints(); idx++)
//            printf("%1.2f, ", this->data_manager->getData(primary_variables[0])[idx]);
//        printf("]\n");
//        
//        printf("temperature:[");
//        for(size_t idx = 0; idx < this->grid->getNumberOfGridPoints(); idx++)
//            printf("%1.2f, ", this->data_manager->getData("temperature")[idx]);
//        printf("]\n");
...
void FDCompressibleFlow::advanceSolution(double dt,MPI_Comm comm)
{
    ...
    vector<double> &Up = this->data_manager->getData(primary_variables[0]+"p");
    
    vector<double> &V = this->data_manager->getData(primary_variables[1]);
    vector<double> &Vp = this->data_manager->getData(primary_variables[1]+"p");
...
//    printf("u:[");
//    for(idx = 0; idx < this->grid->getNumberOfGridPoints(); idx++)
//        printf("%1.2f, ", U[idx]);
//    printf("]\n");
//    
//    printf("\nup:[");
...
//    printf("]\n");
    ...
    for(int i = 0; i <= local_rows; i++)
    {
        for(int j = 0; j <= local_cols; j++)
        {
            ...
            Stencil rho_p = this->data_manager->getStencil("densityp");
            ...
            Up[idx] =  U[idx]*(rho.O/rho_p.O) + dt/rho_p.O * source;
            ...
            Vp[idx] = V[idx]*(rho.O/rho_p.O) + dt/rho_p.O  * source;
